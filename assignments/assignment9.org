...
- Another update: the raw data as output by scapy is in an odd format that necesitated converting it to a hex string before converting _that_ into a printable string in order to make a viewable PNG from the data. This took far longer than I would have liked to figure out, but it's done now and I have the flag, thanks mainly to 'binascii.hexlify()' and then xxd. All my answers (which have been checked on the CTF site) are above.
